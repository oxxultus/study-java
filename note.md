...
---
### 자바 입출력
System.out.println -> 표준 출력으로 출력하라는 것
System.out.println(name + "안녕" ); 자동 줄 바꿈
System.out.print(name + "안녕\\n"); 자동 줄 바꿈 없음
System.out.printf("%s 안녕?%n", name); c언어의 printf와 동일 
- 가능하면\\n이 아닌 %n 사용하라
String phrase = String.format("%s %s %s", name ,name ,name);  정적메서드
---
### 라이브러리
